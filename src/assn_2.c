...
//    which_order = "--best-fit";
    ...
    if ((fp = fopen(file_name, "r+b")) != NULL) {
        ...
        available_list_end = available_list_size + 1;
        ...
        if (output_index_file) {
            ...
            prim_index_end = index_list_size + 1;
    ...
    do {
        ...
        if (strcmp(operation, add_label) == 0) {
            ...
            printf("Invalid Key Word Entered \n");
        }
        fflush(stdin);
    } while (1);
}
...
void find_record(int key)
{
    ...
    else
    {
        ...
        char *output = (char *)malloc(record_size + sizeof(int));
        fread(output, record_size, 1, fp);
        printf("%s\n", output);
        fclose(fp);
    }
}
...
long available_offset_for_any_order(int record_size) {
	...
	for (i = 0; i < available_list_end; i++) {
		if (record_size < available_list[i].size) {
			int j;
			int diff_size = available_list[i].size - record_size - sizeof(int);
			long diff_offset = available_list[i].off + record_size
					+ sizeof(int);
			long offset_location = available_list[i].off;
			if (which_order == first_fit_label) {
				...
				if (diff_size > 0) {
					available_list[available_list_end - 1].size = diff_size;
					available_list[available_list_end - 1].off = diff_offset;
			...
			if (diff_size > 0) {
				available_list[i].size = diff_size;
				available_list[i].off = diff_offset;

				if (which_order == best_fit_label) {
					qsort(available_list, i + 1, sizeof(avail_S),
							comparator_ascending);
					return offset_location;
				} else if (which_order == worst_fit_label) {
					qsort(available_list, available_list_end, sizeof(avail_S),
